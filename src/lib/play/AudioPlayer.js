import { ObjectsExt } from "../util/ObjectsExt.js";
...
export class AudioPlayer {
  ...
  stop() {
    if (ObjectsExt.exists(this.audioContext)) {
      this.audioContext.close();
    }

    if (ObjectsExt.exists(this.analyser)) {
      this.analyser.disconnect();
    }

    if (ObjectsExt.exists(this.workletNode)) {
      this.workletNode.disconnect();
    }

    if (ObjectsExt.exists(this.recorderNode)) {
  ...
  async init() {
    // Initialize audio player - stub for now
    this.initialized = true;
    console.log("AudioPlayer initialized");
  }

  setAudioDataCallback(callback) {
    // Set callback for audio data - stub for now
    this.audioDataCallback = callback;
    console.log("AudioPlayer audio data callback set");
  }

  async stop() {
    // Stop audio player - stub for now
